# Project: SHVAL tool NoT
#
# Author: EFSA-AMU_zancaga
###############################################################################
...
rawSVdata <- read.csv(
	system.file(
		"test_reference/j4_forager_acute/j4_forager_acute.csv",
		package = "shinyshval"
	),
	header = T
)
...
alpha <- 0.01
sim <- 1000
iter <- 1000 #! iter is used nowhere!
...
for (i in 1:4) {
	# IF raw data are available & more than 2 values
	if (
		(sum(is.na(rawSVdata[, i])) != length(rawSVdata[, i])) &
			sum(!is.na(rawSVdata[, i])) > 2
	) {
		stp <- fitdist(
			as.vector(as.numeric(na.omit(rawSVdata[, i]))),
			"norm",
			optim.method = "L-BFGS-B"
		)
		momnts[i, ] <- c(as.vector(stp$estimate[1]), as.vector(stp$estimate[2]), NA)
	} else {
		# Raw data are NOT available, but we have mean and StDev
		if (!is.na(rawSVdata[4, i + 6])) {
			momnts[i, ] <- c(rawSVdata[3, i + 6], rawSVdata[4, i + 6], NA)
		} else {
			# MIN, MAX (and Mean) are available
			if (!is.na(rawSVdata[1, i + 6]) & !is.na(rawSVdata[2, i + 6])) {
				aver <- (rawSVdata[1, i + 6] + rawSVdata[2, i + 6]) / 2
				momnts[i, ] <- c(
					aver,
					(aver - rawSVdata[1, i + 6]) / qnorm(1 - alpha),
					NA
				)
			} else {
				# The best guess (mean or median) is the only info available
				momnts[i, ] <- c(0, 0, NA)
			}
		}
	}
...
if (
	(sum(is.na(rawSVdata[, 5])) != length(rawSVdata[, 5])) &
		sum(!is.na(rawSVdata[, 5])) > 2
) {
	stp <- fitdist(
		as.vector(as.numeric(na.omit(rawSVdata[, 5]))),
		"beta",
		optim.method = "L-BFGS-B",
		lower = c(0, 0)
	)
	...
	# IF min, max & BG are available
	if (
		!is.na(rawSVdata[1, 11]) &
			!is.na(rawSVdata[2, 11]) &
			!is.na(rawSVdata[3, 11])
	) {
		betapar <- get.beta.par(
			p = c(0.01, 0.5, 0.99),
			q = c(rawSVdata[1, 11], rawSVdata[3, 11], rawSVdata[2, 11]),
			show.output = FALSE,
			plot = FALSE
		)
		momnts[5, 1] <- betapar[1]
		...
		# IF min & max are available
		if (!is.na(rawSVdata[1, 11]) & !is.na(rawSVdata[2, 11])) {
			betapar <- get.beta.par(
				p = c(0.01, 0.99),
				q = c(rawSVdata[1, 11], rawSVdata[2, 11]),
				show.output = FALSE,
				plot = FALSE
			)
			momnts[5, 1] <- betapar[1]
			...
			# When only the best guess is available
			momnts[5, 1] <- 0
			momnts[5, 2] <- 0
		}
	}
...
simfit_CONSp <- (if (momnts[3, 1] != 0 & momnts[3, 2] != 0) {
	...
	apply(
		matrix(
			rnorm(sim * sim, mean = momnts[3, 1], sd = momnts[3, 2]),
			nrow = sim,
			ncol = sim,
			byrow = TRUE
		),
...
simfit_CONSs <- (if (momnts[4, 1] != 0 & momnts[4, 2] != 0) {
	...
	apply(
		matrix(
			rnorm(sim * sim, mean = momnts[4, 1], sd = momnts[4, 2]),
			nrow = sim,
			ncol = sim,
			byrow = TRUE
		),
...
for (j in 1:sim) {
	simfit_RUDp <- (if (momnts[1, 1] != 0 & momnts[1, 2] != 0) {
		set.seed(123456 + j)
		exp(rnorm(sim, mean = momnts[1, 1], sd = momnts[1, 2]))
	} else {
		rep(exp(rawSVdata[3, 7]), sim)
	})

	simfit_RUDn <- (if (momnts[2, 1] != 0 & momnts[2, 2] != 0) {
		set.seed(654321 + j)
		exp(rnorm(sim, mean = momnts[2, 1], sd = momnts[2, 2]))
	} else {
		rep(exp(rawSVdata[3, 8]), sim)
	})

	simfit_CONTs <- (if (momnts[5, 1] != 0 & momnts[5, 2] != 0) {
		set.seed(13579 + j)
		rbeta(sim, shape1 = momnts[5, 1], shape2 = momnts[5, 2])
	} else {
		rep(rawSVdata[3, 11], sim)
	})

	SVs <- ((simfit_RUDp * simfit_CONSp) +
		(simfit_RUDn * (simfit_CONSs / simfit_CONTs))) /
		1000
...
# title(main=paste("Shortcut Values' 90th percentile"), cex.main=0.95)
...
paste(
	"The 90th percentile for this particular Shortcut Value is",
	round(qls[2], digits = 3),
	"(CI=",
	round(qls[1], digits = 1),
	"-",
	round(qls[3], digits = 1),
	", CL=95%)"
)
