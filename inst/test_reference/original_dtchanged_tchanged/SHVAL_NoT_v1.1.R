...
# DATA UPLOAD #
rawSVdata <- read.csv(system.file("testdata/original_dtchanged_tchanged/original_dtchanged_tchanged.csv", package = "shinyshval"), header=T)
...
SVs <- ((simfit_RUDp*simfit_CONSp)+(simfit_RUDn*(simfit_CONSs/simfit_CONTs)))/1000
temp <- quantile(SVs, probs=0.9)
SV <- c(SV,temp)
}

simval <- cbind(simfit_RUDp,simfit_RUDn,simfit_CONSp,simfit_CONSs,simfit_CONTs)
